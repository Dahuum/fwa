...
public class SignUpServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        out.println("    <title>Sign Up - Cinema</title>");
        out.println("    <style>");
        ...
        out.println("        button { width: 100%; padding: 10px; background: #007bff; color: white; border: none; border-radius: 4px; cursor: pointer; font-size: 16px; }");
        ...
        if ("exists".equals(error)) {
            out.println("        <div class='error'>Email already exists!</div>");
        } else if ("missing".equals(error)) {
            out.println("        <div class='error'>All fields are required!</div>");
        }
        
        out.println("        <form method='POST' action='/Cinema/signUp'>");
        out.println("            <input type='text' name='firstName' placeholder='First Name' required>");
        out.println("            <input type='text' name='lastName' placeholder='Last Name' required>");
        ...
        out.println("            <a href='/Cinema/signIn'>Already have an account? Sign In</a>");
        out.println("        </div>");
        out.println("    </div>");
        out.println("</body>");
        out.println("</html>");
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        {
            resp.sendRedirect("/Cinema/signUp?error=missing");
            return ;
        }
        ...
        try {
            ...
            resp.sendRedirect("/Cinema/signUp?error=exists");
        }
    }
}